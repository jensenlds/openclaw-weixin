...
import { createReplyDispatcherWithTyping, finalizeInboundContext } from "openclaw/plugin-sdk/reply-runtime";
...
export async function processOneMessage(full, deps) {
    if (!deps?.config) {
        logger.error(`processOneMessage: deps.config is undefined, skipping message from=${full.from_user_id}`);
        deps.errLog("processOneMessage: deps.config is undefined, skip");
        return;
    }
    ...
    // Resolve sender command authorization.
    // When deps.commands is not available (external plugin without full runtime), use a no-op
    // runtime that skips command-specific authorization. Pairing-based DM access is handled
    // by the sender allowlist below.
    const noopRuntime = {
    ...
    const { senderAllowedForCommands, commandAuthorized } = await resolveSenderCommandAuthorizationWithRuntime({
        ...
        runtime: deps.commands ?? noopRuntime,
    });
    ...
    const storePath = resolveStorePath(deps.config.session?.store, {
    ...
    const humanDelay = resolveHumanDelayConfig(deps.config, route.agentId);
