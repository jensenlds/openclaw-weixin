...
export const weixinPlugin = {
    ...
    gateway: {
        startAccount: async (ctx) => {
            ...
            // notifyStart is skipped — it can hang indefinitely in some network environments.
            // The long-poll loop (getUpdates) handles the actual message stream without it.
            /* try {
            ...
            // Ensure runtime is registered so message handlers can access it.
            console.error(`[3ffb1af2bb54-im-bot] startAccount ctx.runtime=${typeof ctx.runtime} ctx.runtime.channel=${typeof ctx.runtime?.channel}`);
            if (ctx.runtime) {

            } else {
                aLog.warn(`ctx.runtime is missing — channelRuntime will be unavailable`);
            }
