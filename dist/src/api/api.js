...
export async function apiGetFetch(params) {
    ...
    try {
        ...
        logger.debug(`${params.label} status=${res.status} raw=${redactBody(res.rawText)}`);
        if (res.status < 200 || res.status >= 300) {
            throw new Error(`${params.label} ${res.status}: ${res.rawText}`);
        }
        return res.rawText;
    }
    catch (err) {
...
/**
 * HTTPS request helper using node:https (avoids native fetch issues in Gateway process).
 */
    ...
    return new Promise((resolve, reject) => {
        const req = https.request(options, (res) => {
            ...
            res.on("end", () => {
                ...
                resolve({ status: res.statusCode, headers: res.headers, rawText });
            });
        });
        req.on("error", (err) => {
            reject(err);
        ...
        if (options.signal) {
            options.signal.addEventListener("abort", () => {
                try { req.destroy(new Error("Aborted")); } catch (_) {}
            });
        }
